<?php

namespace Utils;
//require_once './vendor/autoload.php';
use Firebase\JWT\JWT;

class Auth
{
    private static $secret = 'M1Clav3Sup3rS3cr3t4@'; //Calve unica para codificación y decodificacion
    private static $encryptType = ['HS256'];
    private static $aud = null;
    
    public static function create($data)
    {
        $now = time();
        
        $payload = array(
        	'iat'=>$now,  //hora actual
            'exp' => $now + (60*120), //2 min - Tiempo en el que expira el token
            'aud' => self::Aud(), //Ambiente en el que se encuentra
            'data' => $data,  //datos que queremos guardar codificados.
            'app'=> "API REST Matias Paverini" 
        );
        return JWT::encode($payload, self::$secret);
    }
    
    public static function checkToken(string $tokenHeader)
    {
        $token = "";

        if(strpos($tokenHeader, "Bearer ") == 0 || strpos($tokenHeader, "Bearer: ") == 0) {
            $token = explode(" ", $tokenHeader)[1];
        }

        if(empty($token) || $token == "")
        {
            //echo "El token esta vacio.";
            throw new Exception("El token esta vacio.");
        } 
        // las siguientes lineas lanzan una excepcion, de no ser correcto o de haberse terminado el tiempo       
        try
        {
            $decode = JWT::decode(
            $token,
            self::$secret,
            self::$encryptType
            );
        } 
        catch (ExpiredException $e)
        {
            //echo "Clave fuera de tiempo";
           throw new Exception("Clave fuera de tiempo");
        }
        
        // si no da error,  verifico los datos de AUD que uso para saber de que lugar viene  
        if($decode->aud !== self::Aud())
        {
            //echo "No es el usuario valido";
            throw new Exception("No es el usuario valido");
        }
        return true;
    }
    
   
     public static function getPayload($tokenHeader)
    {
        $token = "";

        if(strpos($tokenHeader, "Bearer ") == 0 || strpos($tokenHeader, "Bearer: ") == 0) {
            $token = explode(" ", $tokenHeader)[1];
        }
        return JWT::decode(
            $token,
            self::$secret,
            self::$encryptType
        );
    }
     public static function getData($tokenHeader)
    {
        $token = "";

        if(strpos($tokenHeader, "Bearer ") == 0 || strpos($tokenHeader, "Bearer: ") == 0) {
            $token = explode(" ", $tokenHeader)[1];
        }

        return JWT::decode(
            $token,
            self::$secret,
            self::$encryptType
        )->data;
    }
    private static function Aud()
    {
        $aud = '';
        
        if (!empty($_SERVER['HTTP_CLIENT_IP'])) {
            $aud = $_SERVER['HTTP_CLIENT_IP'];
        } elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
            $aud = $_SERVER['HTTP_X_FORWARDED_FOR'];
        } else {
            $aud = $_SERVER['REMOTE_ADDR'];
        }
        
        $aud .= @$_SERVER['HTTP_USER_AGENT'];
        $aud .= gethostname();
        
        return sha1($aud);
    }
}